...
export default function BrandLogo({ type = "default", size = "md", className = "", hideText = false }: BrandLogoProps) {
  const sizeClasses = {
    sm: { logo: "w-8 h-8", title: "text-lg", subtitle: "text-[8px] tracking-[0.3em]" },
    md: { logo: "w-9 h-9", title: "text-xl", subtitle: "text-[9px] tracking-[0.3em]" },
    lg: { logo: "w-11 h-11", title: "text-2xl", subtitle: "text-[11px] tracking-[0.4em]" }
  };
    ...
    <div className={`flex items-center gap-3 ${className} group`}>
      ...
      {!hideText && (
        <div className="flex flex-col leading-[0.8]">
          {/* Main Title: GLOBAL */}
          <span className={`${currentSize.title} font-black text-[#22c55e] tracking-tighter italic uppercase drop-shadow-[0_0_10px_rgba(34,197,94,0.4)]`}>
            Global
          </span>
          
          <div className="flex items-center gap-2 mt-1">
            {/* Subtitle: TRUST CASH */}
            <span className={`${currentSize.subtitle} font-bold text-[#facc15] uppercase ml-0.5 drop-shadow-[0_0_8px_rgba(250,204,21,0.3)] whitespace-nowrap`}>
              Trust Cash
            </span>

            {/* Badge */}
            {type !== "default" && (
              <span className="text-[7px] bg-[#22c55e] text-black px-1.5 py-0.5 rounded-md font-black uppercase tracking-tighter">
                {type}
              </span>
            )}
          </div>
